...
export default class KanjiExampleCommand extends Command {
    ...
    private static logger: Logger = new Logger('KanjiExample Command', { useColor: true, useGlobalLogInformationFactories: true })
    ...
    public async execute(activatedCommand: ActivatedCommand): Promise<void> {
        try {
            ...
            KanjiExampleCommand.logger.logSuccess(`KanjiExample command for phrase ${phrase} completed.`)
        }
        catch (err) {
            KanjiExampleCommand.logger.logError(err)
