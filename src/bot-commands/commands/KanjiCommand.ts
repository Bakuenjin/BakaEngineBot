...
export default class KanjiCommand extends Command {
    ...
    private static logger: Logger = new Logger('Kanji Command', { useColor: true, useGlobalLogInformationFactories: true })
    ...
    public async execute(activatedCommand: ActivatedCommand): Promise<void> {
        try {
            ...
            KanjiCommand.logger.logSuccess(`Kanji command for ${kanji} completed.`)

        }
        catch (err) {
            KanjiCommand.logger.logError(err)
