...
export default class JishoCommand implements Command {
    ...
    private static logger: Logger = new Logger('Jisho Command', { useColor: true, useGlobalLogInformationFactories: true })
    ...
    public async execute(activatedCommand: ActivatedCommand): Promise<void> {
        try {
            ...
            JishoCommand.logger.logSuccess(`Jisho command for phrase '${phrase}' completed.`)
        } 
        catch (err) {
            JishoCommand.logger.logError(err)
